...
envelope로 변환합니다. OCR을 새로 수행하거나 문서 내용을 법률지식으로 승인하지
않습니다.
...
OCR PDF와 추출된 full text JSON은 Git 저장소 밖의 private storage에 둡니다.
`--output`이 Git worktree 안이면 명령이 실패합니다.
...
v4.27.0의 공개 안전 보고 형식으로 원문과 private PDF 경로를 제거합니다.
...
PDF 물리 페이지 번호와 인쇄 페이지 표지는 별도 필드로 보존합니다. 표지를 감지하지
못하더라도 물리 페이지를 바꾸거나 추정값을 확정하지 않습니다.

## 실패와 제한

- 암호화 PDF는 먼저 복호화해야 합니다.
- 텍스트 레이어가 비어 있는 페이지는 별도 OCR을 시도하지 않고 누락으로 기록합니다.
- 장·절 경계는 짧고 명시적인 표제만 후보로 잡습니다.
- 표·각주·다단 편집의 읽기 순서는 원본 PDF에 따라 추가 검수가 필요합니다.
- 변환 결과는 전부 private input이며 후속 추출 결과도 `draft` 또는
  `review_required`에서 시작해야 합니다.
