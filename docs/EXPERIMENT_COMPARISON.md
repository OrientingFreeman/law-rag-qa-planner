...
Baseline과 Agent는 동일한 데이터, corpus, 검색 전략, `top_k`, 기준일과 deterministic provider를 사용합니다. 차이는 Agent가 사전 안전 점검, 단계별 trace와 제한된 재시도를 적용한다는 점입니다.

검색 품질과 답변 품질, 안전성은 서로 다른 문제이므로 별도 지표로 기록합니다.
...
`evaluation/datasets/official_core_cases.json`은 61개 사례로 구성됩니다.
...
가짜 조문은 질문의 부정 입력에만 존재합니다. gold 조문이나 corpus 및 법령 관계에는 편입하지 않았습니다.
...
사례 비교는 개선 6개, 악화 0개, 동일 통과 36개, 동일 실패 19개였습니다. `incorrect_outcome`은 15건에서 10건으로 감소했고, `retrieval_miss`, `wrong_top1`, `over_retrieval`은 변하지 않았습니다.

## 4. 해석

Agent의 개선은 검색 순위 향상이 아닙니다. 기준일 없는 질문, 지나치게 넓은 요청과 존재하지 않는 조문을 답변 전에 판정한 결과입니다. 검색 지표가 동일하다는 사실을 함께 제시해야 합니다.

안전성의 대가도 있습니다. 평균 응답시간이 늘었고 답변 가능한 사례를 불필요하게 보류한 비율도 소폭 높아졌습니다. 또한 재시도는 이번 평가에서 어떤 사례도 근거 품질을 개선하지 못했습니다. 이 결과는 재시도 정책을 더 복잡하게 만들기보다, 어떤 실패가 재시도로 해결 가능한지 먼저 분류해야 함을 보여줍니다.
...
`--no-query-rewrite`, `--no-reranking`, `--max-retries 0` 옵션으로 설정 차이를 별도 실험할 수 있습니다.

## 6. 수치의 한계

- 폐쇄형 61개 사례와 deterministic provider에 한정됩니다.
- 인용 정확도 100%는 검색 결과 밖 조문을 인용하지 않았다는 구조적 검증이며 법적 결론 정확도 100%가 아닙니다.
- 응답시간은 실행 장비와 프로세스 상태에 따라 달라집니다.
- 실험 원문은 `evaluation/experiments/`에 저장하지만 Git에는 포함하지 않습니다.
- 검증된 소형 결과는 `evaluation/baselines/v4.16.0_baseline_vs_agent_summary.json`에 보존합니다.
