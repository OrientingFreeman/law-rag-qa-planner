...
최초 실행에서는 `tests/test_api.py`가 현재 홈페이지 문구인 “법무·컴플라이언스를 위한”이 아니라 이전 문구 “기업 법무·컴플라이언스를 위한”을 기대해 1건 실패했다. 현재 사용자 UI를 변경하지 않고 테스트 기대 문구만 실제 HTML과 일치시켰으며, 수정 후 전체 145개가 통과했다.
